...
const CriticalPatientScreen = () => {
  const { criticalOnly } = useRoute().params; 
  const [patients, setPatients] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Fetch all patients or just critical patients
    fetchPatients();
  }, [criticalOnly]);
  ...
  const fetchPatients = async () => {
    try {
      ...
      if (criticalOnly) {
        // Filter only critical patients
        setPatients(data.filter((patient) => patient.critical === true));
      } else {
        setPatients(data); 
      }
      setLoading(false);
